...
if (SAVE_CSV) {
  ...
  raw_stats_col <- c('A', 'Ci', 'Ca', 'gsw', 'PhiPS2', 'iWUE')
  
  stats_col <- c(
    paste0(raw_stats_col, '_avg'),
    paste0(raw_stats_col, '_stderr')
  )
  
  stats_col <- c(
    EVENT_COLUMN_NAME, 'seq_num',
    sort(stats_col)
  )
  ...
  write.csv(all_stats[, stats_col], file = file.path(base_dir, 'all_stats.csv'), row.names = FALSE)
}
